...
def create_random_string(num=100, no_links=1):
    s = ""
    for i in range(0, num):
        s += chr(randint(65, 90)) + " " + str(randint(0, no_links)) + " "
    s = s.replace('0', '')
    return s
...
def create_files(num=1, path="files"):
    file_name = []
    for i in range(0, num):
        file_name.append(path + "/" + str(i + 1) + ".txt")

    for i in range(0, len(file_name)):
        write_file(file_name[i], create_random_string(randint(1, 10), len(file_name)))
    return file_name
...
def read_file(file_path, gui=False):
    file_str = open(file_path).read()
    out_str = ""
    if gui:
        for i in range(len(file_str)):
            if file_str[i].isnumeric():
                continue
            else:
                out_str += file_str[i]
        return out_str
...
def get_adj_mx(files_score):
    mx = {}
    for i in files_score:
        str_file = read_file("files/" + str(i[0]) + ".txt", gui=False)
        file_mx = set([])
        for j in range(len(str_file)):
            if str_file[j].isnumeric():
                file_mx.add(int(str_file[j]))
        if not (file_mx == set() or file_mx is None):
            mx[i[0]] = file_mx
        else:
            mx[i[0]] = None
    return mx
...
def main_fun(input_str="B D", random_files=False):
    if random_files:
        create_return = create_files(5, 'files')
    ...
    # remove files with CosSim = 0
    authorities_mod = {}
    print("Authority Scores: ", authorities)
    for i in authorities:
        for j in sorted_score:
            if j[1] != 0:
                authorities_mod[int(j[0])] = authorities[int(j[0])]
    print(authorities_mod)
    authorities_mod = sorted(authorities_mod.items(), key=lambda kv: kv[1], reverse=True)
    print("Hub Scores: ", hubs)
    print("Authority Scores: ", authorities_mod)
    plt.savefig("vector_space/static/images/graph1.png")
    # plt.show()
    # ==========================

    dict_out = {}
    for i in authorities_mod:
        dict_out[i[0]] = [str(format_float(i[1])), read_file("files/" + str(i[0]) + ".txt", gui=True)]
